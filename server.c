...
const char* lookup(const char* path)
{
    ...
    {
        //set extension to string after dot
        char* extension = strrchr(path, '.');
        
        //check to make sure extension not null 
        // and then look at all possible extensions and return correct type
        if (extension == NULL)
            return NULL;
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    char method[strlen(temp)* sizeof(char)];
    char target[strlen(temp)* sizeof(char)];
    char version[strlen(temp)* sizeof(char)];
    ...
    //ensure proper spacing in request
    if(tokenCount != 3)
    {
        error(400);
        return false;
    ...
    //if it does reset token counter to 0 and use strtok to split target by ?
    else if(strchr(target, '?') != NULL)
    {
        tokenCount = 0;
        char* queryToken = strtok(target, "?");
        
        while (queryToken != NULL)
        {
            if (tokenCount == 0)
            {
                strcpy(abs_path, queryToken);
            }
            else if(tokenCount == 1)
            {
                strcpy(query, queryToken);
            }
...
/**
 * Returns statsttus code's reason phrase.
 *
